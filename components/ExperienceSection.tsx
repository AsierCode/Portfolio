...
const ExperienceSection: React.FC<ExperienceSectionProps> = ({ id, experience, language }) => {
  if (!experience || experience.length === 0) {
    return null;
  }

  const title = uiStrings.experienceTitle[language];
      ...
      <div className="container mx-auto px-6">
        ...
        <div className="space-y-12">
          {experience.map((entry, index) => {
            const description = language === 'es' ? entry.descriptionEs : entry.descriptionEn;
            return (
              <div
                key={index}
                ...
                <div className="flex flex-col sm:flex-row items-start mb-3">
                  ...
                  <div className="w-full">
                    <div className="flex flex-col sm:flex-row justify-between items-baseline">
                      <h3 className="text-xl lg:text-2xl font-semibold text-sky-300">
                        {language === 'es' ? entry.roleEs : entry.roleEn}
                      </h3>
                      <p className="text-sm text-slate-500 mt-1 sm:mt-0 whitespace-nowrap">{entry.years}</p>
                    </div>
                    <p className="text-md lg:text-lg text-slate-300">
                      {language === 'es' ? entry.companyEs : entry.companyEn}
                    </p>
                  </div>
                </div>
